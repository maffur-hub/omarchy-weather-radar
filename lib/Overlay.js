...
// Kp now (the latest observed three-hour block) and the highest NOAA expects
// in the next three days, so the aurora view can say what the sky is doing and
// what is coming.
function parseKp(raw) {
  ...
  if (!isFinite(nowKp) && !isFinite(peakForecast)) return null
  return { nowKp: nowKp, peakForecast: peakForecast, observedAt: latestObservedAt }
}
